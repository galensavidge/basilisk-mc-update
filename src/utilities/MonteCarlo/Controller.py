...
from Basilisk.utilities.MonteCarlo.Dispersions import BaseDispersion
...
class SimulationParameters:
    ...
    creationFunction: Callable

    # Function that executes the simulation
    executionFunction: Callable

    # Function that configures that simulation after dispersions have been
    # applied
    configureFunction: Callable

    # Iterable structure of retention policies to use in the simulation
    retentionPolicies: Iterable[RetentionPolicy]

    # Iterable list of dispersions to apply
    dispersions: Iterable[BaseDispersion]

    # Set to True to disperse model seeds (not Monte Carlo dispersion seeds)
    shouldDisperseSeeds: bool

    # Set to True to archive a copy of this simulation run as a JSON and write
    # a list of failed runs to a .txt file
    shouldArchiveParameters: bool

    # Base config/data file name for this simulation
    filename: str

    # Initial conditions file name for this simulation, if using
    icfilename: str = ''

    # Run index in Monte Carlo cohort
    index: int = 0

    # Set to True to enable verbose console output
    verbose: bool = False

    # A dictionary of sim modifications in the form
    #   {'varPath': 'varValue', ...}
    # Items in this dictionary are treated as "saved parameters" and not
    # overwritten when applying new parameter dispersions.
    modifications: Dict = field(default_factory=dict)  # Avoid mutable defaults

    # A dictionary of human-readable dispersion magnitude strings generated by
    # dispersion classes
    dispersionMag: Dict = field(default_factory=dict)

    # Set to True to save the dispersion magnitudes to a .txt file
    saveDispMag: bool = False

    # Set to True to show the progress bar for this simulation
...
class SimulationExecutor:
    ...
    @classmethod
    def __call__(cls, params: Tuple[SimulationParameters, mp.Queue]):
        """
